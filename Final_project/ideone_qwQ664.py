    ...
    for i, frame in enumerate(frame_data):
        if abs(frame[1] - last_pitch) > 1.0:
            ...
            if abs(frame[1] - next_frame[1]) <= 1.0:
                if last_pitch != 0.0:
                    prediction.append([start, round(frame[0], 3), round(last_sum / count)])
                last_sum = 0.0
                count = 0
                start = frame[0]
            else:                
        ...
        last_sum += frame[1]
        count += 1   
        if (len(prediction)>2) and i>1:          
            if (prediction[len(prediction)-1][0] -prediction[len(prediction)-2][1]) <=0 and prediction[len(prediction)-2][2] == prediction[len(prediction)-1][2]:
                prediction[len(prediction)-2][1]=prediction[len(prediction)-1][1]
                prediction.pop()
